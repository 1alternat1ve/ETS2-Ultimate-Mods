...
const mocks: Mocks = {
  ...
  list_profiles: () => [
    { id: "default", name: " Driver", location: "local", money: 1_250_000, experience: 847_392, truck: "Volvo FH16", last_modified: "2026-05-20T14:30:00Z", mods_count: 42, sii_path: "C:\\Users\\2akkv\\Documents\\Euro Truck Simulator 2\\profiles\\default\\profile_data.sii" },
    { id: "cloud_76561198012345678", name: "Pro Driver", location: "steam_cloud", money: 3_800_000, experience: 2_140_000, truck: "Scania R Streamline", last_modified: "2026-05-19T09:15:00Z", mods_count: 67, sii_path: "D:\\Steam\\userdata\\76561198012345678\\227300\\remote\\profiles\\default\\profile.sii" },
    { id: "vanilla", name: "New Game", location: "local", money: 0, experience: 0, truck: "Scania R", last_modified: "2026-05-10T18:00:00Z", mods_count: 0, sii_path: "C:\\Users\\2akkv\\Documents\\Euro Truck Simulator 2\\profiles\\vanilla\\profile_data.sii" },
  ],
  ...
  apply_mod_order_to_path: () => ({ backup: "profile.sii.bak", build_type: "convoy" }),
  reset_manifest: () => true,
  scan_dlc: () => [
...
export async function invoke<T = unknown>(cmd: string, args?: Record<string, unknown>): Promise<T> {
  if (IS_TAURI) {
    ...
    // Add 8-second timeout — if Rust doesn't respond, reject so UI shows error instead of hanging
...
export const realTauri = {
  ...
  get_game_version: (game_path: string) => invoke<string>("get_game_version", { game_path }),
  is_game_running: () => invoke<boolean>("is_game_running"),
  is_steam_running: () => invoke<boolean>("is_steam_running"),
  launch_game: (game_path: string) => invoke<boolean>("launch_game", { game_path }),
  launch_game_safe_mode: (game_path: string) => invoke<boolean>("launch_game_safe_mode", { game_path }),
  close_game: () => invoke<boolean>("close_game"),

  // DLC
  scan_dlc: (game_path: string) => invoke<DlcInfo[]>("scan_dlc", { game_path }),
  ...
  encrypt_sii_command: (path: string) => invoke<string>("encrypt_sii_command", { path }),
  ...
  // Backups
  list_backups: (backups_path: string) => invoke<BackupInfo[]>("list_backups", { backups_path }),
  create_backup: (profiles_path: string, backups_path: string, name: string, auto: boolean) =>
    invoke<boolean>("create_backup", { profiles_path, backups_path, name, auto }),
  restore_backup: (backup_path: string, profiles_path: string) =>
    invoke<boolean>("restore_backup", { backup_path, profiles_path }),
  delete_backup: (backup_path: string) => invoke<boolean>("delete_backup", { backup_path }),

  // Mods
  scan_mods: (mods_path: string) => invoke<ModGroups>("scan_mods", { mods_path }),
  toggle_mod: (path: string, enabled: boolean) => invoke<boolean>("toggle_mod", { path, enabled }),
  ...
  debug_read_profile: (profile_path: string) => invoke<any>("debug_read_profile", { profile_path }),

  // Download engine
  download_with_progress: (download_url: string, file_dest: string, github_token?: string) =>
    invoke("download_with_progress", { download_url, file_dest, github_token }),
  log_to_file: (message: string) => invoke("log_to_file", { message }),
  extract_archive: (archive_path: string, dest_path: string) =>
    invoke("extract_archive", { archive_path, dest_path }),
  extract_dlc: (archive_path: string, dest: string) =>
    invoke("extract_dlc", { archive_path, dest }),
  copy_to_mods: (src: string, dest: string) =>
    invoke("copy_to_mods", { src, dest }),
  ...
  // Optimize
  clean_game_files: (game_path: string) => invoke<CleanResult>("clean_game_files", { game_path }),
  clean_crash_dumps: (game_path: string) => invoke<CleanResult>("clean_crash_dumps", { game_path }),
  steam_validate: () => invoke<boolean>("steam_validate"),
  apply_buffer_page_fix: (game_path: string) =>
    invoke<BufferPageResult>("apply_buffer_page_fix", { game_path }),

  // Tools
  fix_trailer_stutter: (game_path: string) =>
    invoke<{ downloaded: string; size_mb: number }>("fix_trailer_stutter", { game_path }),
  apply_mod_order_to_path: (profile_path: string, build_type: string) =>
    invoke<{ backup: string; build_type: string }>("apply_mod_order_to_path", { profile_path, build_type }),
